...
function GreenBG() {
  ...
  useEffect(() => {
    ...
    const build = () => {
      unit = Math.min(H, 950);
      baseX = W > 720 ? W * 0.62 : W * 0.5;
      baseY = H * 0.86;
      const branches = [], pads = [], roots = [], grass = [];
      const mkBranch = (x, y, ang, len, w0, w1, g0, g1, z, bendMag) => {
        ...
        for (let k = 0; k <= 14; k++) {
          ...
          const wob = Math.sin(t * 9 + len) * w0 * 0.10;
          pts.push([ix + wob, iy]);
        }
        branches.push({ pts, w0, w1, g0, g1, z, ph: rnd() * 6.28 });
        return [ex, ey, a2];
      };
      const mkPad = (x, y, rx, ry, z, g0) => {
        const leaves = [];
        const n = Math.round((rx * ry) / 26) + 26;
        for (let i = 0; i < n; i++) {
          const a = rnd() * 6.283, rr = Math.sqrt(rnd());
          const dx = Math.cos(a) * rx * rr, dy = Math.sin(a) * ry * rr;
          const lightK = 0.55 - (dx / rx) * 0.22 - (dy / ry) * 0.38;
          leaves.push({
            dx, dy, rot: rnd() * 3.14,
            s: unit * (0.0038 + rnd() * 0.0034),
            g: Math.min(0.98, g0 + rnd() * 0.09),
            k: Math.max(0, Math.min(1, lightK + (rnd() - 0.5) * 0.25))
          });
        }
        pads.push({ x, y, rx, ry, z, g0, leaves, ph: rnd() * 6.28 });
      };
      // twin gnarled trunk, S-curved
      const p1 = mkBranch(baseX, baseY, J(-0.12, 0.12), unit * 0.16, unit * 0.030, unit * 0.022, 0.02, 0.17, 0, 0.6);
      const p2 = mkBranch(p1[0], p1[1], p1[2] + J(-0.5, 0.5), unit * 0.145, unit * 0.022, unit * 0.015, 0.15, 0.32, 0, 0.7);
      mkBranch(baseX + unit * 0.013, baseY, J(-0.25, 0.1), unit * 0.14, unit * 0.015, unit * 0.009, 0.05, 0.22, -0.5, 0.9);
      // low drooping left branch with pads (like the reference)
      const bl1 = mkBranch(p1[0], p1[1], -1.85 + J(-0.2, 0.2), unit * 0.13, unit * 0.013, unit * 0.008, 0.30, 0.44, 0.35, 0.9);
      const bl2 = mkBranch(bl1[0], bl1[1], bl1[2] + 0.7 + J(-0.2, 0.4), unit * 0.10, unit * 0.008, unit * 0.005, 0.42, 0.54, 0.35, 0.8);
      mkPad(bl2[0], bl2[1] - unit * 0.012, unit * 0.056, unit * 0.030, 0.35, 0.52);
      mkPad(bl1[0] - unit * 0.025, bl1[1] - unit * 0.02, unit * 0.042, unit * 0.023, -0.4, 0.48);
      // mid right branch
      const br1 = mkBranch(p2[0], p2[1], 1.45 + J(-0.2, 0.2), unit * 0.11, unit * 0.011, unit * 0.006, 0.44, 0.58, -0.3, 0.8);
      mkPad(br1[0] + unit * 0.012, br1[1] - unit * 0.014, unit * 0.05, unit * 0.027, -0.3, 0.58);
      // apex branches fanning into the dome canopy
      const tops = [-0.95, -0.3, 0.3, 0.95];
      tops.forEach((ta, i) => {
        const tb = mkBranch(p2[0], p2[1], ta + J(-0.15, 0.15), unit * (0.085 + rnd() * 0.05), unit * 0.009, unit * 0.005, 0.56 + i * 0.02, 0.70 + i * 0.02, (i % 2 ? -0.35 : 0.35), 0.7);
        mkPad(tb[0], tb[1] - unit * 0.01, unit * (0.045 + rnd() * 0.018), unit * (0.025 + rnd() * 0.008), (i % 2 ? -0.35 : 0.35), 0.70 + i * 0.03);
      });
      mkPad(p2[0] + J(-1, 1) * unit * 0.012, p2[1] - unit * 0.105, unit * 0.078, unit * 0.040, 0.1, 0.80);
      mkPad(p2[0] - unit * 0.055, p2[1] - unit * 0.068, unit * 0.05, unit * 0.028, -0.2, 0.85);
      mkPad(p2[0] + unit * 0.058, p2[1] - unit * 0.062, unit * 0.048, unit * 0.027, 0.25, 0.89);
      // buttress roots gripping the soil
      for (let i = 0; i < 7; i++) {
        const a = (i / 6 - 0.5) * 2.6 + J(-0.12, 0.12);
        roots.push({
          x0: baseX + Math.sin(a) * unit * 0.010, y0: baseY - unit * 0.016,
          x1: baseX + Math.sin(a) * unit * (0.045 + rnd() * 0.022), y1: baseY + unit * 0.006,
          w: unit * (0.006 + rnd() * 0.006)
        });
      }
      for (let i = 0; i < 12; i++) grass.push({ x: baseX + (rnd() - 0.5) * unit * 0.17, l: unit * (0.008 + rnd() * 0.012), a: (rnd() - 0.5) * 0.9, ph: rnd() * 6.28 });
      pads.sort((a, b) => a.z - b.z);
      branches.sort((a, b) => a.z - b.z);
      tree = { branches, pads, roots, grass };
    };
    ...
    const barkMain = (z) => z < -0.1 ? "#4E4034" : "#6B5947";
    const leafCol = (k) => {
      // dark shade -> lit shade, top-left lit like the photo
      const dr = 23, dg = 71, db = 43, lr = 127, lg = 219, lb = 147;
      return "rgb(" + Math.round(dr + (lr - dr) * k) + "," + Math.round(dg + (lg - dg) * k) + "," + Math.round(db + (lb - db) * k) + ")";
    };

    const drawPot = () => {
      const rx = unit * 0.088, ry = unit * 0.017, ph = unit * 0.052;
      // bowl body
      const grad = ctx.createLinearGradient(baseX - rx, 0, baseX + rx, 0);
      grad.addColorStop(0, "#57534B"); grad.addColorStop(0.35, "#8A857A"); grad.addColorStop(0.65, "#77726A"); grad.addColorStop(1, "#4C4841");
      ctx.fillStyle = grad;
      ctx.beginPath();
      ctx.moveTo(baseX - rx, baseY);
      ctx.bezierCurveTo(baseX - rx * 0.96, baseY + ph * 0.85, baseX - rx * 0.62, baseY + ph, baseX - rx * 0.52, baseY + ph);
      ctx.lineTo(baseX + rx * 0.52, baseY + ph);
      ctx.bezierCurveTo(baseX + rx * 0.62, baseY + ph, baseX + rx * 0.96, baseY + ph * 0.85, baseX + rx, baseY);
      ctx.closePath(); ctx.fill();
      // feet
      ctx.fillStyle = "#3E3B35";
      for (const fx of [-0.36, 0, 0.36]) {
        ctx.beginPath(); ctx.ellipse(baseX + rx * fx, baseY + ph + 3, unit * 0.012, unit * 0.006, 0, 0, 6.283); ctx.fill();
      }
      // rim
      ctx.fillStyle = "#8F8A7E";
      ctx.beginPath(); ctx.ellipse(baseX, baseY, rx, ry, 0, 0, 6.283); ctx.fill();
      ctx.fillStyle = "#241C14";
      ctx.beginPath(); ctx.ellipse(baseX, baseY, rx * 0.86, ry * 0.8, 0, 0, 6.283); ctx.fill();
      // rim highlight
      ctx.strokeStyle = "rgba(235,230,218,.35)"; ctx.lineWidth = 1.2;
      ctx.beginPath(); ctx.ellipse(baseX, baseY, rx, ry, 0, 3.4, 6.0); ctx.stroke();
      // moss specks
      ctx.fillStyle = "rgba(70,190,121,.35)";
      for (let i = 0; i < 8; i++) {
        ctx.beginPath(); ctx.arc(baseX + Math.sin(i * 2.1) * rx * 0.6, baseY + Math.cos(i * 1.7) * ry * 0.4, 1.3, 0, 6.283); ctx.fill();
      }
    };

    const drawBranchSet = (list, g, time) => {
      ctx.lineCap = "round";
      for (const b of list) {
        const t = (g - b.g0) / (b.g1 - b.g0);
        if (t <= 0) continue;
        const e = sm(Math.min(1, t));
        const swayA = reduced ? 0 : Math.sin(time * 0.45 + b.ph) * 0.5;
        const n = Math.max(1, Math.round(e * (b.pts.length - 1)));
        for (let k = 0; k < n; k++) {
          const fr = k / (b.pts.length - 1);
          const w = Math.max(0.9, b.w0 + (b.w1 - b.w0) * fr);
          const sx = swayA * fr * fr;
          const x0 = b.pts[k][0] + sx, y0 = b.pts[k][1], x1 = b.pts[k + 1][0] + sx, y1 = b.pts[k + 1][1];
          // shadow edge, body, highlight: cylindrical shading
          ctx.strokeStyle = "rgba(28,20,14,.6)"; ctx.lineWidth = w;
          ctx.beginPath(); ctx.moveTo(x0 + w * 0.22, y0); ctx.lineTo(x1 + w * 0.22, y1); ctx.stroke();
          ctx.strokeStyle = barkMain(b.z); ctx.lineWidth = w;
          ctx.beginPath(); ctx.moveTo(x0, y0); ctx.lineTo(x1, y1); ctx.stroke();
          ctx.strokeStyle = "rgba(214,196,172,.22)"; ctx.lineWidth = Math.max(0.6, w * 0.32);
          ctx.beginPath(); ctx.moveTo(x0 - w * 0.24, y0); ctx.lineTo(x1 - w * 0.24, y1); ctx.stroke();
        }
      }
    };

    const drawPadSet = (list, g, time) => {
      for (const p of list) {
        const t = (g - p.g0) / 0.1;
        if (t <= 0) continue;
        const e = sm(Math.min(1, t));
        const sw = reduced ? 0 : Math.sin(time * 0.55 + p.ph) * 1.6;
        // canopy under-shadow gives the cloud its volume
        ctx.fillStyle = "rgba(8,26,15," + (0.5 * e).toFixed(3) + ")";
        ctx.beginPath(); ctx.ellipse(p.x + sw, p.y + p.ry * 0.35, p.rx * 1.02 * e, p.ry * 0.95 * e, 0, 0, 6.283); ctx.fill();
        for (const l of p.leaves) {
          if (g < l.g) continue;
          const ls = l.s * sm(Math.min(1, (g - l.g) / 0.05));
          if (ls < 0.3) continue;
          ctx.fillStyle = leafCol(l.k);
          ctx.beginPath(); ctx.ellipse(p.x + l.dx + sw, p.y + l.dy, ls, ls * 0.55, l.rot, 0, 6.283); ctx.fill();
        }
      }
    };
    ...
    const tick = (now) => {
      ...
      drawPot();
      // roots creep over the soil early
      const rg = sm(Math.min(1, g / 0.08));
      if (rg > 0.02) {
        ctx.lineCap = "round";
        for (const r of tree.roots) {
          ctx.strokeStyle = "#5C4B3C"; ctx.lineWidth = r.w * rg;
      ...
      if (seedT < 1 || g < 0.04) {
        ...
        const glowR = 16 + (reduced ? 0 : Math.sin(time * 2.6) * 4);
        const gl = ctx.createRadialGradient(baseX, sy, 0, baseX, sy, glowR);
        gl.addColorStop(0, "rgba(120,235,165,.5)"); gl.addColorStop(0.5, "rgba(70,190,121,.22)"); gl.addColorStop(1, "rgba(70,190,121,0)");
        ctx.fillStyle = gl;
        ctx.beginPath(); ctx.arc(baseX, sy, glowR, 0, 6.283); ctx.fill();
      ...
      // depth-sorted: back canopies, then wood, then front canopies
      drawPadSet(tree.pads.filter(p => p.z < 0), g, time);
      drawBranchSet(tree.branches, g, time);
      drawPadSet(tree.pads.filter(p => p.z >= 0), g, time);
    };
    raf = requestAnimationFrame(tick);
    return () => { killed = true; cancelAnimationFrame(raf); removeEventListener("resize", resize); };
