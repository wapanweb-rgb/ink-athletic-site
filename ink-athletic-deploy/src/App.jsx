...
function GlassCarousel({ products, onOpen, onQuote, onBuy }) {
  ...
  return (
    <div className="carousel-wrap">
      ...
      <div className="gdetail" key={fp.id}>
        <h3>{fp.name}</h3>
        <p>{fp.outcome}</p>
        {fp.buy && fp.weightG > 0 && (
...
function Storefront({ site, products, onOpen, onQuote, onBuy }) {
  ...
  const cats = CATEGORIES.filter(c => c === "Software & Hardware" || products.some(p => (p.category || "") === c));
...
function DustLayer({ active }) {
  ...
  const onMove = (e) => {
    ...
    spawnAt((e.clientX - r.left) / (r.width / 300), (e.clientY - r.top) / (r.height / 293));
  };
  const onEnter = (e) => {
    if (!active || !wrapRef.current) return;
    const r = wrapRef.current.getBoundingClientRect();
    spawnAt((e.clientX - r.left) / (r.width / 300), (e.clientY - r.top) / (r.height / 293), 7);
  };
...
function SocialPyramid({ site }) {
  ...
  const open = (i) => {
    const url = socials[PYR_NETS[i].key];
      ...
      <div className="sp-stage" onPointerDown={onDown} onPointerUp={onUp} onPointerLeave={onUp}>
        <div className={"pyr" + (turning ? " turning" : "")} style={{ "--rot": (steps * 120) + "deg" }}>
          <div className="pyr-rotor">
            {PYR_NETS.map((net, i) => (
              ...
              >
                <span className="pf-logo">
                  <span className="lg-sh"><SocialGlyph net={net.key} /></span>
                  <span className="lg-hi"><SocialGlyph net={net.key} /></span>
                  <span className="lg-main"><SocialGlyph net={net.key} /></span>
                </span>
            ...
            <div className="pyr-gears" aria-hidden="true">
              <svg className="gear g1" viewBox="0 0 100 100">
                <circle cx="50" cy="50" r="30" fill="none" stroke="#B9BDC6" strokeWidth="13" strokeDasharray="6.5 6.6" />
                <circle cx="50" cy="50" r="17" fill="none" stroke="#7A7E86" strokeWidth="5" />
                <circle cx="50" cy="50" r="5" fill="#FF4650" />
              </svg>
              <svg className="gear g2" viewBox="0 0 100 100">
                <circle cx="50" cy="50" r="30" fill="none" stroke="#8F939C" strokeWidth="13" strokeDasharray="8.7 8.9" />
                <circle cx="50" cy="50" r="15" fill="none" stroke="#5C6068" strokeWidth="5" />
                <circle cx="50" cy="50" r="5" fill="#C1121F" />
              </svg>
              <svg className="gear g3" viewBox="0 0 100 100">
                <circle cx="50" cy="50" r="30" fill="none" stroke="#A8ACB5" strokeWidth="13" strokeDasharray="5.2 5.3" />
                <circle cx="50" cy="50" r="16" fill="none" stroke="#6B6F77" strokeWidth="5" />
                <circle cx="50" cy="50" r="5" fill="#FF4650" />
              </svg>
            </div>
          </div>
        ...
        <div className="f-col">
          ...
          <div className="social-row">
            {(site.socials?.instagram) && (
              <a className="sicon" href={site.socials.instagram} target="_blank" rel="noreferrer" aria-label="Instagram" title="Instagram">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
            ...
            {(site.socials?.facebook) && (
              <a className="sicon" href={site.socials.facebook} target="_blank" rel="noreferrer" aria-label="Facebook" title="Facebook">
                <svg viewBox="0 0 24 24" fill="currentColor">
                  <path d="M13.5 21v-7h2.6l.4-3h-3V9.1c0-.87.24-1.46 1.5-1.46h1.6V5.06c-.28-.04-1.23-.12-2.33-.12-2.31 0-3.89 1.41-3.89 4v2.06H7.8v3h2.58v7h3.12z" />
                </svg>
              </a>
            ...
            {(site.socials?.tiktok) && (
              <a className="sicon" href={site.socials.tiktok} target="_blank" rel="noreferrer" aria-label="TikTok" title="TikTok">
                <svg viewBox="0 0 24 24" fill="currentColor">
                  <path d="M16.6 3c.35 1.88 1.55 3.14 3.4 3.36v2.62c-1.27.05-2.42-.32-3.55-1.06v5.87c0 3.6-2.5 5.71-5.42 5.71-2.8 0-5.03-2.1-5.03-5.03 0-2.9 2.3-5.02 5.36-4.9v2.7c-.3-.06-.6-.09-.9-.06-1.24.1-2.06 1-2.06 2.26 0 1.32 1.02 2.32 2.33 2.32 1.4 0 2.34-1 2.34-2.6V3h3.53z" />
                </svg>
              </a>
...
function Nav({ count, cartCount, showCart, onQuoteOpen, onCartOpen, onHome, isHome, ready }) {
...
export default function App() {
  ...
  return (
    <>
      <style>{CSS}</style>
      {!isAdminRoute && (
        <Nav count={count} cartCount={cartCount} showCart={products.some(p => p.buy) || cartCount > 0} onQuoteOpen={() => setDrawer(true)} onCartOpen={() => setCartOpen(true)} onHome={goHome} isHome={!product} ready={belowFold} />
      )}
