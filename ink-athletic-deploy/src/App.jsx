...
const GLYPHS = [
  <g><path d="M22 52c6-3.5 15-4.5 24-3" /><path d="M25 52l-3.5 14M33 51l-2 15M43 50.5l1.5 15M52 50l3.5 13" /><path d="M46 49c4-2 7.5-6 8.5-11" /><path d="M54.5 38c-1.5-7 2.5-13.5 8.5-16.5M54.5 38c3-6.5 9.5-9.5 15.5-9" /><path d="M22 52c-4.5-1-6.5-5-5.5-9.5" /></g>,
  <g><path d="M31 13.5c3-2 6.5-.5 7 3s-2.5 6.5-6 6-5.5-4-4.5-7" /><path d="M34 23c-1.5 9-1 16 .5 24" /><path d="M34 30.5l-10.5 8M34.5 30l11 6.5" /><path d="M34.5 47l-8.5 15.5M34.5 47l9.5 14" /><path d="M19 65L58.5 19.5" /><path d="M58.5 19.5l-6.5 1M58.5 19.5l-1 6.5" /></g>,
  <g><path d="M30 62c-4.5-8-5.5-16-4-24.5" /><path d="M26 38l-4.5-12.5" /><path d="M33 36.5l-2.5-16.5" /><path d="M40 36l1-17.5" /><path d="M47 37.5l4.5-14" /><path d="M52.5 42l7.5-8.5" /><path d="M30 62c8.5 3.5 16.5 2.5 21.5-3 2-6 2-12.5 1-17.5" /></g>,
  <g><path d="M40 40c1.5-4 6-5.5 9.5-2.5 4 4 2.5 11-3 13.5-7.5 3-15.5-2-16.5-10-1.5-10.5 8-18.5 18.5-17.5 12 1 19.5 12 17.5 23.5" /><path d="M11 39.5l-6.5.5M40.5 11.5l-.5-7M69 41l7-.5M39.5 69.5l.5 6" /></g>,
  <g><path d="M14 40.5c8.5-9.5 17.5-13.5 26.5-13.5s17.5 4.5 25.5 13c-8.5 8-17.5 12-26.5 12s-17.5-4-25.5-11.5z" /><path d="M35 39.5c1-3.5 5-5 8-3s3.5 6.5.5 9-8 1-9.5-2.5" /><path d="M62 47.5c3.5 5.5 2.5 10-2 13.5M28.5 52.5L23.5 65" /><path d="M14 40.5l-6.5-2.5" /></g>,
  <g><path d="M14 58c6.5-6 12.5-6.5 18.5-1.5s12.5 5.5 18.5-.5 9.5-11 8-18.5c-1.5-6.5-6.5-9.5-11.5-7.5" /><path d="M47.5 30.5c-.5 1 .5 2 1.5 1.5s.5-2-.5-2" /><path d="M46 33l-6.5 4.5" /></g>,
  <g><path d="M26.5 58c-5.5-6.5-6.5-14.5-2-20.5 4-5.5 11-7 17-4l7.5 4" /><path d="M48.5 38c5 2.5 8.5.5 10.5-3.5" /><path d="M58.5 34.5c3.5-1 6.5.5 8 3.5" /><path d="M30 58.5l-2.5 10.5M38 58l.5 11" /><path d="M24.5 44.5L12 40" /></g>,
  <g><path d="M11.5 33.5l9.5-10 9 10.5 9.5-10 9 10 9.5-10.5 9 10" /><path d="M12 50.5l9-9.5 9.5 9 9-9.5 9.5 9.5 9-9 9.5 9.5" /></g>
];

function ProcessCard({ s, i, open, total }) {
  const delay = open ? i * 0.22 : (total - 1 - i) * 0.14;
  // Hand-daubed glyph scatter: 3-7 marks, random pick/placement/tilt per
  // card (stable for the session), pulsing out of sync like embers.
  const marks = React.useMemo(() => {
    const n = 3 + Math.floor(Math.random() * 5);
    // Anchor-plus-scatter: some marks drift toward a random cluster point,
    // others land anywhere — reads far more "human" than uniform spread.
    const cx = 15 + Math.random() * 60, cy = 15 + Math.random() * 55;
    return Array.from({ length: n }, () => {
      const clustered = Math.random() < 0.45;
      const left = clustered ? Math.max(2, Math.min(88, cx + (Math.random() - 0.5) * 46)) : 2 + Math.random() * 88;
      const top = clustered ? Math.max(4, Math.min(74, cy + (Math.random() - 0.5) * 44)) : 4 + Math.random() * 72;
      return {
        g: Math.floor(Math.random() * GLYPHS.length),
        left, top,
        size: 18 + Math.pow(Math.random(), 1.6) * 40,
  ...
  return (
    <div className="scardwrap">
      <div className={"scard" + (open ? " sopen" : "")} style={{ transitionDelay: delay + "s" }}>
        <span className="hinge" aria-hidden="true"><i className="pin" /><i className="rod" /><i className="pin" /></span>
        {marks.map((m, k) => (
          <span key={k} className="sglyph" aria-hidden="true"
            style={{ left: m.left + "%", top: m.top + "%", width: m.size + "px", height: m.size + "px", transform: "rotate(" + m.rot + "deg) scaleX(" + (m.flip ? -1 : 1) + ")" }}>
            <span className="g-dark"><svg viewBox="0 0 80 80">{GLYPHS[m.g]}</svg></span>
            <span className="g-hot" style={{ animationDelay: m.delay + "s", animationDuration: m.dur + "s" }}><svg viewBox="0 0 80 80">{GLYPHS[m.g]}</svg></span>
          </span>
        ))}
...
function Process({ site }) {
  ...
  // Auto-fold: once the reader scrolls past the bottom of the open scroll,
  // it folds itself back up.
  useEffect(() => {
    if (!open) return;
    const onScroll = () => {
      ...
      if (!el) return;
      const r = el.getBoundingClientRect();
      if (r.bottom < (innerHeight || 800) * 0.18) setOpen(false);
    };
    addEventListener("scroll", onScroll, { passive: true });
        ...
        <div className="f-col">
          ...
          <div className="social-row">
            {["instagram", "facebook", "tiktok"].map(net => (site.socials?.[net]) && (
              <a key={net} className="sicon" href={site.socials[net]} target="_blank" rel="noreferrer"
