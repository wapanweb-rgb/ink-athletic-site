...
function GlyphMark({ m, open }) {
  ...
  return (
    <span className="sglyph" aria-hidden="true"
      style={{ left: m.left + "%", top: m.top + "%", width: m.size + "px", height: m.size + "px" }}>
...
function ProcessCard({ s, i, open, total }) {
  ...
  // Glyphs live in a reserved right-hand zone (never over the text, always
  // inside the card). Random slots + jitter: random-looking, no overlaps.
  const marks = React.useMemo(() => {
    const slots = [];
    for (const col of [6, 38, 70]) for (const row of [3, 28, 53, 76]) slots.push({ col, row });
    for (let k = slots.length - 1; k > 0; k--) { const j = Math.floor(Math.random() * (k + 1)); [slots[k], slots[j]] = [slots[j], slots[k]]; }
    const n = 3 + Math.floor(Math.random() * 5); // 3-7 marks
    return slots.slice(0, n).map(sl => ({
      g: Math.floor(Math.random() * GLYPHS.length),
      left: sl.col + (Math.random() - 0.5) * 10,
      top: sl.row + (Math.random() - 0.5) * 8,
      size: 16 + Math.pow(Math.random(), 1.4) * 16,
  ...
  return (
    <div className="scardwrap">
      <div className={"scard" + (open ? " sopen" : "")} style={{ transitionDelay: delay + "s" }}>
        ...
        <span className="glyphzone" aria-hidden="true">
