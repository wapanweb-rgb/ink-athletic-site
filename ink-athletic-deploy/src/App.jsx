...
function Founder({ site }) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const span = vh * 0.42 + r.height * 0.15;     // travel distance to fully-out
      let p = 1 - Math.max(0, dist - hold) / span;
      p = Math.max(0, Math.min(1, p));
      p = p * p * (3 - 2 * p); // ease
      const off = ((1 - p) * 108).toFixed(2) + "%";
