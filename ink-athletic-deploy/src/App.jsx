...
function GreenBG() {
  ...
  useEffect(() => {
    ...
    let branches = [], pads = [], grass = [], baseX = 0, baseY = 0;
    const seedStart = performance.now();
    const rnd = Math.random;

    const build = () => {
      branches = []; pads = []; grass = [];
      baseX = W > 720 ? W * 0.60 : W * 0.5;
      baseY = H * 0.93;
      const unit = Math.min(H, 900);
      const grow = (x, y, ang, len, w, depth, g0) => {
        const g1 = Math.min(1, g0 + 0.13 + rnd() * 0.1);
        const bend = (rnd() - 0.5) * (depth === 0 ? 0.5 : 1.1);
        ...
        for (let k = 0; k <= 12; k++) {
          ...
          const wob = Math.sin(t * 7 + depth * 2.3 + len) * w * 0.07 * (depth > 0 ? 1 : 0.5);
          pts.push([ix + wob, iy]);
        }
        branches.push({ pts, w0: w, w1: Math.max(1, w * 0.55), g0, g1, depth, ph: rnd() * 6.28 });
        if (depth === 0) {
          grow(ex, ey, a2 + (rnd() - 0.5) * 0.5, len * 0.82, w * 0.72, 1, g1 - 0.02);
          grow(ex, ey, a2 + (rnd() < 0.5 ? -1 : 1) * (0.8 + rnd() * 0.4), len * 0.55, w * 0.5, 2, g1);
        } else if (depth < 5 && w > 1.4) {
          const kids = rnd() < 0.72 ? 2 : 3;
          for (let i = 0; i < kids; i++) {
            const dir = i === 0 ? -1 : (i === 1 ? 1 : (rnd() < 0.5 ? -1 : 1));
            const na = a2 + dir * (0.45 + rnd() * 0.55) + (rnd() - 0.5) * 0.3;
            grow(ex, ey, na, len * (0.6 + rnd() * 0.2), w * 0.62, depth + 1, g1 - 0.04 + rnd() * 0.04);
          }
          if (rnd() < 0.5) {
            const [bx, by] = pts[4 + Math.floor(rnd() * 6)];
            pads.push({ x: bx, y: by, r: unit * (0.005 + rnd() * 0.004), g0: Math.min(1, g1 + 0.02), bl: rnd() < 0.35, ph: rnd() * 6.28 });
          }
        } else {
          const n = 5 + Math.floor(rnd() * 5);
          for (let i = 0; i < n; i++) {
            pads.push({
              x: ex + (rnd() - 0.5) * unit * 0.055, y: ey - rnd() * unit * 0.03,
              r: unit * (0.011 + rnd() * 0.019), g0: Math.min(1, g1 + rnd() * 0.08),
              bl: rnd() < 0.1, ph: rnd() * 6.28
            });
          }
        }
      };
      grow(baseX, baseY, (rnd() - 0.5) * 0.24, unit * 0.17, unit * 0.02, 0, 0.03);
      for (let i = 0; i < 14; i++) grass.push({ x: baseX + (rnd() - 0.5) * unit * 0.26, l: unit * (0.012 + rnd() * 0.02), a: (rnd() - 0.5) * 0.9, ph: rnd() * 6.28 });
    };
    ...
    let g = 0;
    const BARK = ["#4A3A2C", "#524236", "#59493C", "#5F4F44", "#66564C", "#6C5C54"];
    const FOLIAGE = ["#1E5C38", "#2E9E5B", "#3FB56C", "#57C87F"];
    const tick = (now) => {
      if (killed) return;
      raf = requestAnimationFrame(tick);
      const time = now / 1000;
      ...
      ctx.fillStyle = "rgba(30,24,20,.85)";
      ctx.beginPath(); ctx.ellipse(baseX, baseY + 5, 92, 14, 0, 0, 6.283); ctx.fill();

      const gg = Math.min(1, g / 0.1);
      if (gg > 0.01) {
        ctx.strokeStyle = "rgba(70,190,121,.5)"; ctx.lineWidth = 1.4;
        for (const s of grass) {
          const sw = reduced ? 0 : Math.sin(time * 1.2 + s.ph) * 0.06;
          ctx.beginPath(); ctx.moveTo(s.x, baseY);
          ctx.quadraticCurveTo(s.x + Math.sin(s.a + sw) * s.l * 0.6, baseY - s.l * 0.6 * gg, s.x + Math.sin(s.a + sw) * s.l, baseY - s.l * gg);
          ctx.stroke();
        }
      }

      if (seedT < 1) {
        const yy = 86 + (baseY - 16 - 86) * (seedT * seedT);
        ctx.save(); ctx.translate(baseX, yy); ctx.rotate(seedT * 4);
        ctx.fillStyle = "#6B4F35"; ctx.beginPath(); ctx.ellipse(0, 0, 5, 7, 0, 0, 6.283); ctx.fill();
        ctx.fillStyle = "rgba(255,255,255,.25)"; ctx.beginPath(); ctx.ellipse(-1.4, -2, 1.4, 2.2, 0, 0, 6.283); ctx.fill();
        ctx.restore();
      } else if (g < 0.04) {
        ctx.fillStyle = "#6B4F35"; ctx.beginPath(); ctx.ellipse(baseX, baseY - 9, 5, 7, 0.4, 0, 6.283); ctx.fill();
        const pulse = 0.25 + 0.15 * Math.sin(time * 2.4);
        ctx.fillStyle = "rgba(70,190,121," + pulse.toFixed(3) + ")";
        ctx.beginPath(); ctx.arc(baseX, baseY - 9, 11, 0, 6.283); ctx.fill();
      }
      ...
      for (const b of branches) {
        ...
        if (t <= 0) continue;
        const e = Math.min(1, t); const s = e * e * (3 - 2 * e);
        const swayA = reduced ? 0 : Math.sin(time * 0.5 + b.ph) * 0.55 * (b.depth * 0.6 + 0.2);
        const n = Math.max(1, Math.round(s * (b.pts.length - 1)));
        ctx.strokeStyle = BARK[Math.min(b.depth, 5)];
        for (let k = 0; k < n; k++) {
          const fr = k / (b.pts.length - 1);
          const sx = swayA * fr * fr;
          ctx.lineWidth = Math.max(0.8, b.w0 + (b.w1 - b.w0) * fr);
          ctx.beginPath();
          ctx.moveTo(b.pts[k][0] + sx, b.pts[k][1]);
          ctx.lineTo(b.pts[k + 1][0] + sx, b.pts[k + 1][1]);
          ctx.stroke();
        }
      }
      for (const p of pads) {
        ...
        if (t <= 0) continue;
        const e = Math.min(1, t); const s = e * e * (3 - 2 * e);
        const sw = reduced ? 0 : Math.sin(time * 0.7 + p.ph) * 1.5;
        if (p.bl) { ctx.fillStyle = "rgba(255,80,92,.85)"; }
        else { ctx.fillStyle = FOLIAGE[Math.floor((p.ph * 10) % 4)]; ctx.globalAlpha = 0.8; }
        ctx.beginPath(); ctx.arc(p.x + sw, p.y, Math.max(0.5, p.r * s), 0, 6.283); ctx.fill();
        ctx.globalAlpha = 1;
      }
    };
    raf = requestAnimationFrame(tick);
...
function EcoCard({ p, i }) {
  ...
  return (
    <div ref={ref} className={"eco-card reveal" + (inView ? " in" : "") + (i % 2 ? " right" : "")}>
